      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-xl bg-white rounded-3xl overflow-hidden shadow-2xl p-6 md:p-8 space-y-6 z-10 animate-fade-in-up"
          >
            <button
              onClick={() => setQuickViewProduct(null)}
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <div className="w-full sm:w-1/2 h-52 bg-slate-50 rounded-2xl flex items-center justify-center p-4 border border-slate-100">
                <img
                  src={getOptimizedUrl(quickViewProduct.imageUrl)}
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-4">
              <div className="flex flex-col">
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">
                  {language === 'ta' ? 'இருப்பு நிலை' : 'Status'}
                </span>
                <span className="text-xs font-black text-emerald-700">
                  {quickViewProduct.stock > 0 ? t('in_stock') : t('out_of_stock')}
                </span>
              </div>
              <button
                ...
                className="px-8 py-3.5 bg-emerald-600 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-emerald-700 transition-all flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
